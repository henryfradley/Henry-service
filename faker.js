...
let createTable = 'CREATE TABLE IF NOT EXISTS campsites(propertyId INT NOT NULL AUTO_INCREMENT, hostedBy VARCHAR(100), nearby VARCHAR(100),fullDescription VARCHAR(2000), campSiteArea VARCHAR(200), essentials VARCHAR(200), amenities VARCHAR(200), activities VARCHAR(200), terrain VARCHAR(200), PRIMARY KEY (propertyId));';
...
for (var i = 0; i < 1; i++) {
  ...
  var queryStr = `INSERT INTO campsites VALUES (propertyId, '${hostedBy}', '${nearby}', '${fullDescription}', '${JSON.stringify(campSiteArea)}', '${JSON.stringify(essentials)}', '${JSON.stringify(amenities)}', '${JSON.stringify(activities)}', '${terrain}');`;
